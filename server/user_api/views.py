...
def create_user(request):
    ...
    if serializer.is_valid():
        serializer.save()
        return Response({'message': 'Create User Success !!','success': True,'data': serializer.data}, status=status.HTTP_201_CREATED)

    return Response({'message': 'Create User Failed','success': False}, status=status.HTTP_400_BAD_REQUEST)
